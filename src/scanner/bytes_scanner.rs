...
impl<'a> BytesScanner<'a> {
    ...
    fn scan_dom(&mut self) -> Result<bool> {
        ...
        if self.ch == ascii::SLA {
            ...
            if start == 0 {
                return Err(self.err(format!("illegal dom-tag-identifier, near character {}.", self.ch as char), pos));
            }
            self.consume_whitespace();
        ...
        while self.ch != ascii::EOF {
            ...
            if let Some(_) = self.find_delimiter(TokenKind::LDelimiter) {
                ...
                if self.source[offs] == ascii::ATS {
                    ...
                    self.tok_buf.offer(Token(TokenKind::Data, offs + 1, end));
                }
